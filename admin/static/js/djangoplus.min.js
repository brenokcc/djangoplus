...
function scrollTop(dontScrool){
    if(dontScrool!=true && top!=null) $("body").animate({scrollTop: 0}, 1000);
}

function popup(url){

    stack.push([url, $( "#app-modal-content" ).children().detach()]);
...
function closePopup(redirect_to){
    ...
    if(stack.length>0 || url.indexOf('select=')>=0){
        ...
        if(item!=null) {
            ...
            if(redirect_to) loadUrl(redirect_to, true);
